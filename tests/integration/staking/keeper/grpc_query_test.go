...
func TestGRPCQueryUnbondingDelegation(t *testing.T) {
	...
	_, _, _, err := f.stakingKeeper.Undelegate(ctx, addrAcc2, valAddr, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
...
func TestGRPCQueryDelegatorUnbondingDelegations(t *testing.T) {
	...
	assert.NilError(t, err1)
	_, _, _, err := f.stakingKeeper.Undelegate(ctx, addrAcc, valAddr1, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
	valAddr2, err1 := sdk.ValAddressFromBech32(addrVal2)
	assert.NilError(t, err1)
	_, _, _, err = f.stakingKeeper.Undelegate(ctx, addrAcc, valAddr2, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
...
func TestGRPCQueryRedelegations(t *testing.T) {
	...
	_, _, _, err = f.stakingKeeper.BeginRedelegation(ctx, addrAcc1, val1bz, val2bz, math.LegacyNewDecFromInt(rdAmount))
	assert.NilError(t, err)
	applyValidatorSetUpdates(t, ctx, f.stakingKeeper, -1)
...
func TestGRPCQueryValidatorUnbondingDelegations(t *testing.T) {
	...
	assert.NilError(t, err)
	_, _, _, err = f.stakingKeeper.Undelegate(ctx, addrAcc1, valbz, math.LegacyNewDecFromInt(undelAmount))
	assert.NilError(t, err)
	applyValidatorSetUpdates(t, ctx, f.stakingKeeper, -1)
