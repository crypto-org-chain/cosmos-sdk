...
// ValidatorQueue queries debug information about the validator unbonding queue
func (k Querier) ValidatorQueue(ctx context.Context, req *types.QueryValidatorQueueRequest) (*types.QueryValidatorQueueResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "empty request")
	}
	
	entries, pageRes, totalCount, totalValidators, err := k.Keeper.getValidatorQueueDebugInfo(ctx, req.Pagination)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	
	return &types.QueryValidatorQueueResponse{
		Entries:         entries,
		Pagination:      pageRes,
...
// GetValidatorQueueDebugInfo returns debug information about the validator queue
func (k Keeper) getValidatorQueueDebugInfo(ctx context.Context, pagination *query.PageRequest) ([]types.ValidatorQueueEntry, *query.PageResponse, uint64, uint64, error) {
	store := k.storeService.OpenKVStore(ctx)
	
	queueStore := prefix.NewStore(runtime.KVStoreAdapter(store), types.ValidatorQueueKey)
	
	var entries []types.ValidatorQueueEntry
	totalValidators := uint64(0)
	
	// Use paginated iteration
	pageRes, err := query.Paginate(queueStore, pagination, func(key []byte, value []byte) error {
		// Parse the queue key to get time and height
		fullKey := append(types.ValidatorQueueKey, key...)
		keyTime, keyHeight, err := types.ParseValidatorQueueKey(fullKey)
		if err != nil {
			return err
		}
		
		// Unmarshal the validator addresses
		addrs := types.ValAddresses{}
		if err := k.cdc.Unmarshal(value, &addrs); err != nil {
			return err
		}
		
		entry := types.ValidatorQueueEntry{
			UnbondingTime:       keyTime,
			UnbondingHeight:     keyHeight,
			ValidatorAddresses:  addrs.Addresses,
		}
		
		entries = append(entries, entry)
		totalValidators += uint64(len(addrs.Addresses))
	...
	// Count total entries for debug info
	...
	return entries, pageRes, totalCount, totalValidators, nil
}
